...
def run_experiment(events, kernel, T, K, spatio_bound, Wt, Ws, dt, ds, seed=0):
    ...
    mean_init = torch.tensor(([[0.2]]))
    sigma_init = torch.tensor(([[0.1]]))
    mean_time_init = torch.tensor(([[0.5]]))
    sigma_time_init = torch.tensor(([[0.3]]))
    ...
    mean_time_train = torch.tensor(([[results['param_kernel_temporal'][0]]]))
    sigma_time_train = torch.tensor(([[results['param_kernel_temporal'][1]]]))

    kernel_params_train = [mean_train, sigma_train,
                            mean_time_train, sigma_time_train]
...
n_dim, events, spatio_bound, K = read_real_data("data_NCEDC/real_data_NCEDC_1970-1972", T)

kernel = ["inverse_power_law", "truncated_gaussian"]
...
df.to_csv('results/exp3_RD_POW-TG.csv', index=False)