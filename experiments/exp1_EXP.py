...
kernel = ["truncated_gaussian", "truncated_exponential"]
kernel_length = [Ws, Wt]
grad_kernel = ["truncated_gaussian", "truncated_exponential"]
...
kernel_params_time = torch.tensor(([[decay]]))
...
def run_experiment(baseline, alpha, kernel_params_time, kernel_params_space_x,\
    ...
    decay_init = kernel_params_time + v
...
true_param = {'baseline': mu, 'alpha': alpha, 'mean': mean, 'sigma': sigma, 'decay': decay}
...
def compute_norm2_error(s):
    return np.sqrt(np.array([(s[param] - s[f'param_{param}'])**2
                            for param in ['baseline', 'alpha', 'mean', 'sigma', 'decay']]).sum())


df['err_norm2'] = df.apply(
    lambda x: compute_norm2_error(x), axis=1)

df.to_csv('results/exp1_EXP.csv', index=False)